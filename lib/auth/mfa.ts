...
export class MFAManager {
  ...
  async verifyMFA(
    ...
    try {
      ...
      if (!method || method === "totp") {
        if (
          config.totp_secret &&
          (await this.verifyTOTPCode(config.totp_secret, code))
        ) {
  ...
  async isMFARequired(userId: string): Promise<boolean> {
    try {
      ...
      return (
        ...
        profile?.subscription_tier === "educational"
      );
    } catch (error) {
  ...
  private async generateTOTPCode(
    secret: string,
    time: number,
  ): Promise<string> {
  ...
  private async getMFAConfiguration(
    userId: string,
  ...
  private async updateMFAConfiguration(
    userId: string,
