...
export class SessionManager {
  ...
  async initializeSession(): Promise<SessionState> {
    try {
      ...
      if (error) {
        ...
        return { user: null, session: null, loading: false, error: error.message }
      }
   ...
   */
  async signIn(email: string, password: string, rememberMe = false): Promise<SessionState> {
    try {
      ...
      return { user: null, session: null, loading: false, error: 'Authentication failed' }
    } catch (error) {
   ...
   */
  async signUp(email: string, password: string, metadata?: Record<string, any>): Promise<SessionState> {
    try {
