...
export class SecurityMiddleware {
  ...
  async handle(request: NextRequest): Promise<NextResponse | null> {
    try {
      ...
      if (this.options.enableRateLimit) {
        const rateLimitResult = await this.applyRateLimit(request, session?.user?.id)
        if (rateLimitResult) {
  ...
  private async createSecurityContext(
    ...
    if (session?.user) {
      ...
      const lockStatus = await this.securityManager.isAccountLocked(session.user.id)
      securityFlags.accountLocked = lockStatus.locked

      // Analyze suspicious activity
      const activityAnalysis = await this.securityManager.analyzeSuspiciousActivity(
        session.user.id,
        request
      )
    ...
    return {
      ...
      session,
      csrfToken: session?.user ? this.generateCSRFToken(session.user.id) : undefined,
      securityFlags,
    }
  }

  private async applyRateLimit(request: NextRequest, userId?: string): Promise<NextResponse | null> {
    ...
    if (pathname.startsWith('/api/auth')) {
      ...
      rateLimitType = userId ? await this.getAIRateLimitType(userId) : 'ai_generation_free'
  ...
  private async validateCSRF(request: NextRequest, userId?: string): Promise<{ valid: boolean; token?: string }> {
  ...
  private generateCSRFToken(userId: string): string {
    const secret = this.options.csrfSecret || process.env.CSRF_SECRET || 'default-secret'
  ...
  private async sanitizeRequest(request: NextRequest): Promise<{ safe: boolean; threats?: string[] }> {
    try {
      ...
      for (const [key, value] of searchParams.entries()) {
        ...
        if (sanitized !== value) {
          threats.push(`URL parameter '${key}' contains potentially malicious content`)
        }
      }
        ...
        if (contentType.includes('application/json')) {
          try {
            ...
            if (sanitizationResult.modified) {
              threats.push('Request body contains potentially malicious content')
            }
          } catch (error) {
  ...
  private sanitizeObject(obj: any, depth = 0): { sanitized: any; modified: boolean } {
    if (depth > 10) {
  ...
  private applySecurityHeaders(response: NextResponse, request: NextRequest): void {
  ...
  private handlePreflight(request: NextRequest, response: NextResponse): NextResponse {
    const origin = request.headers.get('origin')
    const trustedOrigins = this.options.trustedOrigins || []

    // Allow requests from trusted origins
    if (origin && (trustedOrigins.includes(origin) || this.isLocalhost(origin))) {
