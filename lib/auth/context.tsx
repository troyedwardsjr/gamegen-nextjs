...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const verifyMFA = useCallback(async (code: string, method?: 'totp' | 'backup_code') => {
    if (!state.user) {
  ...
  const resetPassword = useCallback(async (email: string): Promise<AuthResult> => {
    try {
      const { error } = await sessionManager.supabase.auth.resetPasswordForEmail(email, {
  ...
  const hasPermission = useCallback((permission: string): boolean => {
    ...
    const sessionPermissions = (state.session as any).permissions as string[] || []
  ...
  const getSubscriptionStatus = useCallback((): 'active' | 'canceled' | 'past_due' | 'none' => {
