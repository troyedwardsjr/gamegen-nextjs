...
export class EmailVerificationManager {
  private supabase = typeof window !== 'undefined' ? createAuthClient() : createServerSupabaseClient()
   ...
   */
  async checkVerificationStatus(userId?: string): Promise<EmailVerificationStatus> {
    try {
      const supabase = await createServerSupabaseClient()
      ...
      const canResend = !lastSent || (Date.now() - lastSent.getTime()) > this.RESEND_COOLDOWN
      
      const nextResendTime = lastSent && !canResend 
  ...
  async requestEmailChange(newEmail: string): Promise<EmailVerificationResult> {
    try {
      ...
      console.log(`[EMAIL] Email change requested for ${user.email} -> ${newEmail}`)
  ...
  async cancelEmailChange(): Promise<EmailVerificationResult> {
    ...
    console.log('[EMAIL] Email change cancellation requested (handled by Supabase expiry)')
    return { success: true }
  }
