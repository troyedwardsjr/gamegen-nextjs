...
export function useSupabaseQuery<T>(
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof DatabaseError ? err : new DatabaseError(String(err)),
      );
      setData(null);
    } finally {
      setLoading(false);
...
export function useGame(gameId: string) {
  ...
  return useSupabaseQuery(() => gameService.findById(gameId), [gameId], {
    enabled: !!gameId,
  });
}
...
export function useCollaborationSession(gameId: string) {
  const [session, setSession] =
    useState<Tables<"collaboration_sessions"> | null>(null);
...
export function useGameAnalytics(
  ...
  return useSupabaseQuery(async () => {
    ...
    if (error)
      throw new DatabaseError("Failed to fetch game analytics", error.code);
    ...
    const averageCompletion = sessions?.length
      ? sessions.reduce(
          (acc, session) => acc + (session.completion_percentage || 0),
          0,
        ) / sessions.length
      : 0;
...
export function useCredits() {
  ...
  const useCredits = useCallback(
    async (amount: number) => {
      ...
      const updatedProfile = await profileService.updateCredits(
        user.id,
        amount,
      );
