...
export class SupabaseTestSuite {
  ...
  async runAllTests(): Promise<{
    ...
    await this.runTest(
      ...
      this.testAdminClientCreation.bind(this),
    );
  ...
  private async runTest(
    ...
    try {
      ...
      logger.error(`❌ ${name} failed (${duration.toFixed(2)}ms)`, {
        error: errorMessage,
      });
    }
  }
  ...
  private async testProfileService(): Promise<void> {
    ...
    const profile = await profileService.findById(
...
export function validateSupabaseEnvironment(): {
  ...
  if (supabaseUrl.includes("127.0.0.1") || supabaseUrl.includes("localhost")) {
    if (process.env.NODE_ENV === "production") {
      ...
      recommendations.push(
        "Using local Supabase - ensure local instance is running",
      );
    }
  }
  ...
  if (poolMax > 20) {
    recommendations.push(
      "DB_POOL_MAX > 20 may be too high for most applications",
    );
  }
