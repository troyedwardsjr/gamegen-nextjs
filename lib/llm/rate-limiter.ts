...
export class RateLimiter {
  ...
  getStatus(userId?: string): RateLimitState {
    ...
    if (userId) {
      ...
      if (userLimit) {
        const totalRequests = userLimit.windows.reduce((sum, w) => sum + w.requests, 0)
    ...
    return {
      ...
      is_exceeded: totalRequests >= this.config.requests_per_minute || totalTokens >= this.config.tokens_per_minute
    }
  }
  ...
  getTimeUntilReset(userId?: string): number {
    const now = new Date()
    const windowEnd = new Date(this.globalWindow.start.getTime() + this.windowSizeMs)
    
    if (userId) {
      const userLimit = this.userLimits.get(userId)
      if (userLimit && userLimit.windows.length > 0) {
        const oldestWindow = userLimit.windows[0]
        const userWindowEnd = new Date(oldestWindow.start.getTime() + this.windowSizeMs)
   ...
   */
  private checkGlobalLimit(requestCount: number, tokenCount: number, now: Date): boolean {
    // Update current window if needed
    this.updateGlobalWindow(now)
    
    const wouldExceedRequests = this.globalWindow.requests + requestCount > this.config.requests_per_minute
    const wouldExceedTokens = this.globalWindow.tokens + tokenCount > this.config.tokens_per_minute
    
    // Check burst limit if configured
    if (this.config.burst_limit) {
      const wouldExceedBurst = this.globalWindow.requests + requestCount > this.config.burst_limit
      if (wouldExceedBurst) {
        return false
      }
    }
   ...
   * Update global usage
   */
  private updateGlobalUsage(requestCount: number, tokenCount: number, now: Date): void {
    this.updateGlobalWindow(now)
  ...
  private updateUserWindows(userLimit: UserRateLimit, now: Date): void {
    ...
    const currentWindowAge = now.getTime() - userLimit.current_window.start.getTime()
    
    if (currentWindowAge >= this.windowSizeMs / 4) { // Create new window every quarter minute
  ...
  private cleanupOldWindows(now: Date): void {
    ...
    for (const [userId, userLimit] of this.userLimits.entries()) {
      userLimit.windows = userLimit.windows.filter(window => 
        window.start.getTime() > cutoff
      )
      
      // Remove user entries with no recent activity
      if (userLimit.windows.length === 0 && 
