...
export class BillingTracker {
  ...
  constructor(config: BillingConfig) {
    ...
    console.info('[BillingTracker] Billing tracker initialized', { enabled: config.enabled })
  }
  
  /**
   * Check if user has sufficient credits for a request
   */
  async checkCredits(userId: string, estimatedTokens: number, providerId: string): Promise<boolean> {
    if (!this.config.enabled || !this.config.credit_system_enabled) {
      return true
    }
    
    const estimatedCost = this.calculateCost(
      { prompt_tokens: estimatedTokens, completion_tokens: estimatedTokens, total_tokens: estimatedTokens * 2 },
      providerId,
      userId
    )
    
    const balance = await this.getCreditBalance(userId)
    const availableCredits = balance.available - (this.creditReservations.get(userId) || 0)
   ...
   */
  async reserveCredits(userId: string, estimatedTokens: number, providerId: string): Promise<string> {
    if (!this.config.enabled || !this.config.credit_system_enabled) {
  ...
  async recordUsage(
    ...
    const actualCost = this.calculateCost(response.usage, response.provider_id, userId)
    ...
    if (this.config.enabled && this.config.credit_system_enabled && this.config.auto_deduct_credits) {
  ...
  async releaseReservation(userId: string, reservationId: string): Promise<void> {
    ...
    if (currentReservation > 0) {
      ...
      console.info('[BillingTracker] Reservation released', { userId, reservationId })
    }
  }
   ...
   */
  async addCredits(userId: string, amount: number, source: string = 'manual'): Promise<CreditBalance> {
    ...
    console.info('[BillingTracker] Credits added', { userId, amount, newBalance })
   ...
   */
  private calculateCost(usage: TokenUsage, providerId: string, userId?: string): number {
    if (!this.config.enabled) {
      return 0
    }
    
    const providerRates = this.config.cost_per_token[providerId] || { input: 0.000003, output: 0.000015 }
    
    const inputCost = (usage.prompt_tokens / 1_000_000) * providerRates.input
    const outputCost = (usage.completion_tokens / 1_000_000) * providerRates.output
   ...
   */
  private async deductCredits(userId: string, amount: number, reservationId?: string): Promise<void> {
    const currentBalance = await this.getCreditBalance(userId)
    
    if (currentBalance.available < amount) {
      throw new InsufficientCreditsError(userId, amount, currentBalance.available)
    }
    ...
    const currentReservation = this.creditReservations.get(userId) || 0
    this.creditReservations.set(userId, Math.max(0, currentReservation - amount))
    
    // Log credit transaction
    await this.logCreditTransaction(userId, -amount, 'debit', reservationId || 'usage')
  }
  
  /**
   * Update credit reservation in database
   */
  private async updateCreditReservation(userId: string, amount: number): Promise<void> {
    const currentBalance = await this.getCreditBalance(userId)
    const newReserved = Math.max(0, currentBalance.reserved + amount)
  ...
  private async logCreditTransaction(
    ...
    if (error) {
      console.error('[BillingTracker] Failed to log credit transaction:', error)
    }
  }
  ...
  private async createDefaultBalance(userId: string): Promise<CreditBalance> {
    ...
    if (error) {
      console.error('[BillingTracker] Failed to create default balance:', error)
    }
