...
export class LLMLogger {
  ...
  async logRequest(
    ...
    const logEntry: RequestLog = {
      ...
      request: this.config.log_requests
        ? this.sanitizeRequest(request)
        : ({} as GenerationRequest),
      response:
        this.config.log_responses && response
  ...
  private async storeLogs(logs: RequestLog[]): Promise<void> {
    ...
    if (error) {
      ...
      if (
        this.config.async_logging &&
        this.logBuffer.length < this.config.buffer_size * 2
      ) {
  ...
  private async storePerformanceMetrics(
    ...
    if (error) {
      ...
      if (
        this.config.async_logging &&
        this.performanceBuffer.length < this.config.buffer_size * 2
      ) {
  ...
  private async storeSecurityEvents(events: SecurityEvent[]): Promise<void> {
    ...
    if (error) {
      ...
      if (
        this.config.async_logging &&
        this.securityBuffer.length < this.config.buffer_size * 2
      ) {
  ...
  private sanitizeRequest(request: GenerationRequest): GenerationRequest {
    ...
    if (sanitized.messages) {
      sanitized.messages = sanitized.messages.map((msg: any) => ({
        ...
        content:
          ...
          msg.content.length > this.config.max_payload_size
            ? msg.content.substring(0, this.config.max_payload_size) +
              "...[truncated]"
            : msg.content,
      }));
    }
    ...
    if (
      ...
      sanitized.system_prompt.length > this.config.max_payload_size
    ) {
      sanitized.system_prompt =
        sanitized.system_prompt.substring(0, this.config.max_payload_size) +
        "...[truncated]";
    }
  ...
  private sanitizeResponse(response: GenerationResponse): GenerationResponse {
    ...
    if (
      sanitized.content &&
      sanitized.content.length > this.config.max_payload_size
    ) {
      sanitized.content =
        sanitized.content.substring(0, this.config.max_payload_size) +
        "...[truncated]";
    }
  ...
  private aggregatePerformanceMetrics(metrics: PerformanceMetrics[]): {
    ...
    const totalTokens = metrics.reduce(
      (sum, m) => sum + m.tokens_used.total_tokens,
      0,
    );
