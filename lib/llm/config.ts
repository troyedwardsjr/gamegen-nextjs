...
export class LLMConfigManager {
  ...
  static fromEnvironment(): LLMConfigManager {
    const config: Partial<LLMSystemConfig> = {
      ...
      max_concurrent_requests: parseInt(process.env.LLM_MAX_CONCURRENT_REQUESTS || '10'),
      health_check_interval: parseInt(process.env.LLM_HEALTH_CHECK_INTERVAL || '30000'),
    ...
    if (claudeApiKey) {
      config.providers!.claude = {
        ...
        config: {
          ...
          rate_limit: {
            requests_per_minute: parseInt(process.env.ANTHROPIC_REQUESTS_PER_MINUTE || '100'),
            tokens_per_minute: parseInt(process.env.ANTHROPIC_TOKENS_PER_MINUTE || '100000')
          },
          health_check_interval: parseInt(process.env.ANTHROPIC_HEALTH_CHECK_INTERVAL || '60000'),
        ...
        circuit_breaker: {
          failure_threshold: parseInt(process.env.ANTHROPIC_FAILURE_THRESHOLD || '5'),
          reset_timeout: parseInt(process.env.ANTHROPIC_RESET_TIMEOUT || '60000'),
          monitor_window: parseInt(process.env.ANTHROPIC_MONITOR_WINDOW || '300000'),
          half_open_max_calls: parseInt(process.env.ANTHROPIC_HALF_OPEN_MAX_CALLS || '3')
        },
        rate_limit: {
          requests_per_minute: parseInt(process.env.ANTHROPIC_REQUESTS_PER_MINUTE || '100'),
          tokens_per_minute: parseInt(process.env.ANTHROPIC_TOKENS_PER_MINUTE || '100000'),
   ...
   */
  updateProviderConfig(providerId: string, updates: Partial<ProviderConfiguration>): void {
   ...
   */
  addProvider(config: Omit<ProviderConfiguration, 'created_at' | 'updated_at'>): void {
  ...
  removeProvider(providerId: string): void {
    ...
    if (this.config.fallback_chain.includes(providerId)) {
      throw new Error(`Cannot remove provider ${providerId}: it's in the fallback chain`)
    }
    
    // Check if it's the default provider
    if (this.config.default_provider === providerId) {
      throw new Error(`Cannot remove provider ${providerId}: it's the default provider`)
    }
  ...
  setProviderEnabled(providerId: string, enabled: boolean): void {
    ...
    console.info('[LLMConfig] Provider enabled status changed', { providerId, enabled })
  }
  ...
  getConfigSummary(): {
    ...
    const lastUpdated = Math.max(
      ...Object.values(this.config.providers).map(p => p.updated_at.getTime())
    )
  ...
  private validateConfig(): void {
    ...
    if (!defaultProvider) {
      throw new Error(`Default provider '${this.config.default_provider}' not found`)
    }
    
    if (!defaultProvider.enabled) {
      throw new Error(`Default provider '${this.config.default_provider}' is disabled`)
    }
    ...
    for (const providerId of this.config.fallback_chain) {
      ...
      if (!provider.enabled) {
        console.warn(`[LLMConfig] Fallback provider '${providerId}' is disabled`)
      }
    }
  ...
  private validateImportedConfig(config: any): void {
    ...
    for (const [providerId, providerConfig] of Object.entries(config.providers)) {
      try {
        ...
        throw new Error(`Invalid provider config for '${providerId}': ${error.message}`)
      }
    }
  }
   ...
   */
  private getConfigDiff(oldConfig: LLMSystemConfig, newConfig: LLMSystemConfig): Record<string, any> {
    const diff: Record<string, any> = {}
    
    // Check top-level changes
    const topLevelFields = ['default_provider', 'fallback_chain', 'max_concurrent_requests', 'health_check_interval', 'failover_enabled']
    for (const field of topLevelFields) {
      if (JSON.stringify(oldConfig[field as keyof LLMSystemConfig]) !== JSON.stringify(newConfig[field as keyof LLMSystemConfig])) {
        diff[field] = {
    ...
    // Added providers
    const addedProviders = [...newProviderIds].filter(id => !oldProviderIds.has(id))
    if (addedProviders.length > 0) {
      diff.added_providers = addedProviders
    }
    
    // Removed providers
    const removedProviders = [...oldProviderIds].filter(id => !newProviderIds.has(id))
