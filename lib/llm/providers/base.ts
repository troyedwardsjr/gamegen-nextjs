...
export abstract class BaseProvider implements LLMProvider {
  ...
  protected abstract _generateInternal(request: GenerationRequest): Promise<GenerationResponse>
      ...
      const response = await pRetry(
        ...
        },
        {
          ...
          onFailedAttempt: (error) => {
            console.warn(`Attempt ${error.attemptNumber} failed for ${this.id}:`, error.message)
          }
        }
      )
   ...
   */
  async generateStream(request: GenerationRequest, callback: StreamCallback): Promise<void> {
    const startTime = Date.now()
    ...
    try {
      const isHealthy = await Promise.race([
        ...
        new Promise<boolean>((_, reject) => 
          setTimeout(() => reject(new Error('Health check timeout')), this._config.timeout)
        )
      ])
      
      this._healthStatus = isHealthy ? ProviderHealthStatus.HEALTHY : ProviderHealthStatus.UNHEALTHY
  ...
  async getMetrics(): Promise<ProviderMetrics> {
    return {
      ...
      average_response_time: this._metrics.total_requests > 0 
  ...
  protected validateRequest(request: GenerationRequest): void {
    ...
    if (request.tools && !this.capabilities.includes(LLMCapability.FUNCTION_CALLING)) {
      throw new LLMError(
  ...
  protected updateMetrics(
    ...
    const totalResponseTime = this._metrics.average_response_time * (this._metrics.total_requests - 1)
    this._metrics.average_response_time = (totalResponseTime + responseTime) / this._metrics.total_requests
