...
export class ProviderManager {
  ...
  async destroy(): Promise<void> {
    ...
    const destroyPromises = Array.from(this.providers.values()).map((p) =>
      p.destroy(),
    );
   ...
   */
  private async responseTimeSelection(
    providers: LLMProvider[],
  ): Promise<LLMProvider> {
  ...
  private async attemptFailover(
    ...
    this.log("warn", "Attempting failover", {
    ...
    for (const fallbackId of this.config.fallback_chain) {
      ...
      try {
        ...
        const hasCapabilities = request.tools
          ? fallbackProvider.capabilities.includes(
    ...
    this.log("error", "All failover attempts failed", {
