...
export class ClaudeProvider extends BaseProvider {
   ...
   * Format messages for Claude API
   */
  private formatClaudeMessages(messages: LLMMessage[]): Anthropic.MessageParam[] {
    return messages
   ...
   */
  private formatClaudeContent(content: string | LLMContentBlock[]): string | Anthropic.ContentBlock[] {
    ...
    return content.map(block => {
      ...
      throw new LLMError('Unsupported content block type', 'INVALID_CONTENT', this.id)
    })
  }
   ...
   */
  private mapClaudeStopReason(reason: string | null): GenerationResponse['finish_reason'] {
    switch (reason) {
      case 'end_turn':
      case 'stop_sequence':
        return 'stop'
  ...
  getModelPricing(): { input: number; output: number } {
    ...
    return pricing[this._config.model as keyof typeof pricing] || pricing['claude-3-5-sonnet-20241022']
  }
