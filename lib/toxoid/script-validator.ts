...
export class ToxoidScriptValidator {
  ...
  private async validateToxoidAPI(
    ...
    lines.forEach((line, index) => {
      ...
      if (line.includes('.getComponent(') && !this.hasNullCheck(lines, index)) {
        result.warnings.push({
          ...
          message: 'Component access without null check may cause runtime errors',
    ...
    result.metrics.apiUsageCount = Object.keys(apiUsage).reduce((acc, key) => {
  ...
  private async validateSecurity(
    ...
    lines.forEach((line, index) => {
      ...
      const suspiciousPatterns = [
        ...
        { pattern: 'insertAdjacentHTML', severity: 'warning' as const, score: 15 },
        { pattern: 'document.write', severity: 'error' as const, score: 25 },
        { pattern: '__proto__', severity: 'warning' as const, score: 20 },
        { pattern: 'constructor.constructor', severity: 'error' as const, score: 30 }
      ];
  ...
  private async validatePerformance(
    ...
    if (metrics.cyclomaticComplexity > this.performanceThresholds.maxCyclomaticComplexity) {
      result.warnings.push({
        line: 0,
        column: 0,
  ...
  private async validateBestPractices(
    ...
    const hasErrorHandling = context.script.includes('try') && context.script.includes('catch');
    if (!hasErrorHandling) {
      result.warnings.push({
        line: 0,
        column: 0,
    ...
    if (!hasLogging) {
      result.suggestions.push('Consider adding console.log statements for debugging');
    }
   ...
   */
  private calculatePerformanceMetrics(context: ValidationContext): PerformanceMetrics {
  ...
  private checkPerformanceAntiPatterns(
    ...
    lines.forEach((line, index) => {
      ...
      if (line.includes('+=') && line.includes('"') && this.isInLoop(lines, index)) {
        result.warnings.push({
          line: lineNumber,
          column: line.indexOf('+='),
   ...
   */
  private validateSystemCreation(line: string, lineNumber: number, result: ScriptValidationResult): void {
    if (!line.includes('"') || !line.includes(',')) {
      result.errors.push({
        line: lineNumber,
        column: line.indexOf('Toxoid.System.create'),
        message: 'System.create requires name, query, phase, and callback parameters',
        severity: 'error',
   ...
   */
  private validateObserverCreation(line: string, lineNumber: number, result: ScriptValidationResult): void {
    if (!line.includes('{') || !line.includes('name') || !line.includes('query')) {
      result.errors.push({
        line: lineNumber,
        column: line.indexOf('Toxoid.Observer.create'),
        message: 'Observer.create requires configuration object with name, query, events, and callback',
        severity: 'error',
   ...
   */
  private validateComponentAccess(line: string, lineNumber: number, result: ScriptValidationResult): void {
   ...
   */
  private checkECSBestPractices(context: ValidationContext, result: ScriptValidationResult): void {
    // Check if systems are properly scoped
    if (context.script.includes('Toxoid.System.create') && 
    ...
    if (context.script.includes('.add(') && 
        !context.script.includes('.getComponent(')) {
      result.suggestions.push('Components should be accessed after being added');
    }
  }
  ...
  private hasNullCheck(lines: string[], lineIndex: number): boolean {
    ...
    return nextLines.some(line => 
      line.includes('if') && (line.includes('!') || line.includes('null') || line.includes('undefined'))
    );
  }

  private isInLoop(lines: string[], lineIndex: number): boolean {
    for (let i = lineIndex - 1; i >= 0; i--) {
      if (lines[i].includes('for') || lines[i].includes('while') || lines[i].includes('.forEach')) {
        return true;
      }
      if (lines[i].includes('}')) {
        break;
  ...
  private calculateScores(result: ScriptValidationResult): void {
    ...
    let performanceReduction = result.errors.length * 10 + result.warnings.length * 5;
    let securityReduction = result.errors.filter(e => e.code.includes('SECURITY')).length * 15;
    let maintainabilityReduction = result.warnings.length * 3;

    result.metrics.performanceScore = Math.max(0, result.metrics.performanceScore - performanceReduction);
    result.metrics.securityScore = Math.max(0, result.metrics.securityScore - securityReduction);
    result.metrics.maintainabilityScore = Math.max(0, result.metrics.maintainabilityScore - maintainabilityReduction);

    // Generate suggestions based on scores
    if (result.metrics.performanceScore < 70) {
      result.suggestions.push('Consider optimizing performance-critical code sections');
    }
