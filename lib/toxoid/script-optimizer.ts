...
export class ToxoidScriptOptimizer {
  ...
  ): Promise<ScriptOptimizationResult> {
    try {
      ...
      for (const pass of passes) {
        try {
          ...
          if (
            ...
            pass.name.includes("Performance")
          ) {
      ...
      return {
        ...
        optimizedScript,
        compressionRatio:
          context.originalScript.length / optimizedScript.length,
      ...
      throw new Error(
        `Script optimization failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  private async buildOptimizationContext(
    ...
    try {
      ...
      throw new Error(
        `Failed to parse script: ${error instanceof Error ? error.message : "Parse error"}`,
      );
    }
  ...
  private async analyzeCode(
    ...
    // Analyze functions
    const functionMatches = context.originalScript.matchAll(
      /function\s+(\w+)\s*\([^)]*\)\s*{[^}]*}/g,
    );
    ...
    for (const match of functionMatches) {
      ...
      functions.push({
        ...
        callCount: this.countFunctionCalls(
          context.originalScript,
          functionName,
        ),
    ...
    variablePatterns.forEach(({ pattern }) => {
      ...
      for (const match of matches) {
        ...
        variables.push({
          ...
          isReassigned: this.isVariableReassigned(
            context.originalScript,
            varName,
            varType,
          ),
    ...
    loopPatterns.forEach(({ pattern, type }) => {
      ...
      for (const match of matches) {
        loops.push({
          ...
          hasBreak: this.hasBreakStatement(
            context.originalScript,
            match.index!,
          ),
        });
    ...
    toxoidAPIs.forEach((api) => {
      ...
      if (matchArray.length > 0) {
        apiCalls.push({
          ...
          lines: matchArray.map((match) =>
            this.findLineNumber(context.originalScript, match.index!),
          ),
          isInLoop: matchArray.some((match) =>
            this.isInLoop(context.originalScript, match.index!),
          ),
        });
      }
    });
    ...
    this.optimizationPasses = [
      ...
      },
      {
        ...
        apply: (context) => {
          ...
          optimized = optimized.replace(letPattern, (match, varName, value) => {
            if (
              !this.isVariableReassigned(context.originalScript, varName, "let")
            ) {
      ...
      },
      {
        ...
        apply: (context) => {
          ...
          if (
            context.targetRuntime !== "quickjs" ||
            context.optimizationLevel === "aggressive"
          ) {
    ...
    this.quickJSOptimizations = [
      ...
      },
      {
        ...
        apply: (context) => {
          ...
          while (
            (match = propertyAccessPattern.exec(context.originalScript)) !==
            null
          ) {
            ...
            propertyAccesses.set(
              access,
              (propertyAccesses.get(access) || 0) + 1,
            );
          }
      ...
      },
      {
        ...
        apply: (context) => {
          ...
          const variableDeclarations =
            optimized.match(/(?:var|let|const)\s+(\w+)/g) || [];

          variableDeclarations.forEach((declaration) => {
            const varName = declaration.split(/\s+/)[1];
            const usageCount = (
              optimized.match(new RegExp(`\\b${varName}\\b`, "g")) || []
            ).length;
  ...
  private selectOptimizationPasses(
    ...
    if (
      context.targetRuntime === "quickjs" ||
      context.targetRuntime === "both"
    ) {
      passes = [
        ...passes,
        ...this.quickJSOptimizations.filter(
          (pass) =>
  ...
  private async calculatePerformanceGains(
    ...
    const memoryReduction = Math.max(
      0,
    ...
    if (optimized.includes("for (let") && !optimized.includes("forEach"))
      speedImprovement += 10; // Loop optimization
    if (optimized.includes("const ") && !original.includes("const "))
      speedImprovement += 5; // Variable optimization
  ...
  private estimateExecutionMetrics(analysis: CodeAnalysis): ExecutionMetrics {
    const totalApiCalls = analysis.apiCalls.reduce(
      (sum, api) => sum + api.count,
      0,
    );
    const loopComplexity = analysis.loops.reduce(
      (sum, loop) => sum + loop.estimatedIterations,
      0,
    );
    ...
    return {
      ...
      systemCallCounts: analysis.apiCalls.reduce(
        (acc, api) => {
  ...
  private calculateAPICallFrequency(
    analysis: CodeAnalysis,
  ): Record<string, number> {
    return analysis.apiCalls.reduce(
      (acc, api) => {
        acc[api.api] = api.count;
  ...
  private categorizeLoopComplexity(
    iterations: number,
  ...
  private suggestLoopOptimization(loop: LoopInfo): string {
    if (loop.type === "forEach")
      return "Consider using traditional for loop for better performance";
    if (loop.isNested)
      return "Consider loop restructuring or caching for nested loops";
    if (loop.estimatedIterations > 1000)
      return "Consider breaking large loops into smaller chunks";
  ...
  private identifyOptimizationOpportunities(
    ...
    // Memory suggestions
    if (analysis.memoryUsage.estimatedHeapSize > 10000000) {
      // > 10MB
      suggestions.push({
        type: "memory",
