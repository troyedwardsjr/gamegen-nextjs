...
export class ToxoidScriptSecurity {
   ...
   */
  async scanScript(script: string, constraints?: ScriptConstraints): Promise<SecurityScanResult> {
    ...
    return {
      passed: riskScore < 30 && !issues.some(issue => issue.severity === 'critical'),
      issues,
   ...
   */
  sanitizeScript(script: string): { sanitized: string; modifications: string[] } {
    let sanitized = script;
    const modifications: string[] = [];

    // Remove dangerous function calls
    const dangerousPatterns = [
      { pattern: /eval\s*\([^)]*\)/g, replacement: '/* eval() call removed */', desc: 'Removed eval() call' },
      { pattern: /Function\s*\([^)]*\)/g, replacement: '/* Function() constructor removed */', desc: 'Removed Function() constructor' },
      { pattern: /setTimeout\s*\([^)]*\)/g, replacement: '/* setTimeout() removed */', desc: 'Removed setTimeout() call' },
      { pattern: /setInterval\s*\([^)]*\)/g, replacement: '/* setInterval() removed */', desc: 'Removed setInterval() call' },
      { pattern: /XMLHttpRequest/g, replacement: '/* XMLHttpRequest removed */', desc: 'Removed XMLHttpRequest usage' },
      { pattern: /fetch\s*\(/g, replacement: '/* fetch() call removed */ (', desc: 'Removed fetch() call' }
    ];
    ...
    const objectAccessPatterns = [
      { pattern: /window\./g, replacement: '/* window access removed */.', desc: 'Removed window object access' },
      { pattern: /document\./g, replacement: '/* document access removed */.', desc: 'Removed document object access' },
      { pattern: /global\./g, replacement: '/* global access removed */.', desc: 'Removed global object access' },
      { pattern: /process\./g, replacement: '/* process access removed */.', desc: 'Removed process object access' },
      { pattern: /require\s*\(/g, replacement: '/* require() removed */ (', desc: 'Removed require() call' },
      { pattern: /import\s*\(/g, replacement: '/* dynamic import removed */ (', desc: 'Removed dynamic import' }
    ];
...
(function() {
  ...
  validateSandboxCompatibility(script: string): {
    ...
    const incompatiblePatterns = [
      { pattern: /class\s+\w+\s+extends/, issue: 'Class inheritance may have limited support in QuickJS' },
      { pattern: /async\s+function|\basync\s+\(/, issue: 'Async/await support may be limited in QuickJS' },
      { pattern: /import\s+.*\s+from/, issue: 'ES6 imports not supported in QuickJS sandbox' },
      { pattern: /export\s+(default\s+)?/, issue: 'ES6 exports not supported in QuickJS sandbox' },
      { pattern: /Symbol\(/, issue: 'Symbol usage may be limited in QuickJS' },
      { pattern: /Proxy\(/, issue: 'Proxy objects may not be fully supported in QuickJS' },
      { pattern: /WeakMap|WeakSet/, issue: 'WeakMap/WeakSet may have limited support in QuickJS' }
    ];
    ...
    if (script.includes('forEach')) {
      recommendations.push('Consider using for loops instead of forEach for better performance');
    }

    if (script.includes('arrow function') || script.includes('=>')) {
      recommendations.push('Arrow functions are supported but regular functions may have better performance');
    }

    if (script.includes('const') || script.includes('let')) {
      recommendations.push('Use var for better QuickJS compatibility if needed');
    }
  ...
  private async buildSecurityContext(
    ...
    return {
      ...
      constraints: constraints || {
        ...
        forbiddenPatterns: Array.from(this.criticalPatterns.map(p => p.source)),
   ...
   * Calculate risk score
   */
  private calculateRiskScore(issues: SecurityIssue[], context: SecurityContext): number {
    let score = 0;
  ...
  private hasResourceExhaustionRisk(code: string): boolean {
    return this.hasInfiniteLoopRisk(code) || 
