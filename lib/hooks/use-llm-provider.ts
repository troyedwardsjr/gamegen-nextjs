...
export interface LLMProviderActions {
  ...
  updateProviderConfig: (providerId: string, config: Partial<ProviderConfiguration>) => Promise<void>
...
export function useLLMProvider(options: UseLLMProviderOptions = {}): LLMProviderState & LLMProviderActions {
  ...
  const updateProviderConfig = useCallback(async (
    ...
    try {
      ...
      const response = await fetch(`/api/llm/providers/${providerId}/config`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      console.error('[useLLMProvider] Error updating provider config:', error)
      setState(prev => ({
        ...prev,
  ...
  const getProviderMetrics = useCallback(async (
    ...
    try {
      const response = await fetch(`/api/llm/providers/${providerId}/metrics`)
      ...
      console.error('[useLLMProvider] Error fetching provider metrics:', error)
      return null
    }
  }, [])
  ...
  const resetCircuitBreaker = useCallback(async (
    ...
    try {
      ...
      const response = await fetch(`/api/llm/providers/${providerId}/reset-circuit`, {
        method: 'POST'
      })
      ...
      console.error('[useLLMProvider] Error resetting circuit breaker:', error)
      setState(prev => ({
        ...prev,
  ...
  useEffect(() => {
    ...
    if (autoRefresh && refreshInterval > 0) {
      refreshIntervalRef.current = setInterval(fetchProviderData, refreshInterval)
    }
 ...
 */
export function useProviderMonitor(providerId: string, options: { refreshInterval?: number } = {}) {
...
export function useProviderHealth() {
  ...
  const healthSummary = {
    ...
    circuitOpen: providers.filter(p => p.circuit_breaker_state === 'open').length,
    available: providers.filter(p => 
      p.health_status === 'healthy' && p.circuit_breaker_state === 'closed'
    ).length
  }
  
  const healthPercentage = providers.length > 0 
    ? (healthSummary.healthy / providers.length) * 100 
    : 0
  
  const overallStatus: ProviderHealthStatus = 
    healthSummary.available === 0 ? 'offline' :
    healthPercentage >= 80 ? 'healthy' :
