...
import { ToxoidWasmLoader, initializeToxoid, getToxoidGameState } from '@/lib/toxoid/wasm-loader';
import { ToxoidInputManager, initializeInput, destroyInput } from '@/lib/toxoid/input-manager';
import { 
    ...
    <div className="text-center space-y-4">
      ...
      <div className="space-y-2">
        ...
        <div className="text-sm text-white/60">
          {progress < 0.2 ? 'Initializing WASM...' : 
           progress < 0.6 ? 'Loading game engine...' :
           progress < 0.9 ? 'Setting up scripting...' : 'Almost ready...'}
        </div>
      </div>
...
const DebugOverlay: React.FC<{ gameState: ToxoidGameState }> = ({ gameState }) => (
  <div className="absolute top-2 left-2 space-y-1 z-20">
    <GlassmorphicBadge variant={gameState.isRunning ? "success" : "danger"} size="sm">
      {gameState.isRunning ? (gameState.isPaused ? "PAUSED" : "RUNNING") : "STOPPED"}
    </GlassmorphicBadge>
    
    <GlassmorphicBadge variant="glass" size="sm">
...
export const ToxoidEngine = forwardRef<ToxoidEngineRef, ToxoidEngineProps>(({
  ...
  }, [width, height, enableScripting, debugMode, autoStart, initialScript, onReady, onError, onScriptError]);
  ...
  const executeScript = useCallback(async (code: string): Promise<boolean> => {
    if (!loaderRef.current) {
    ...
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Script error';
      if (onScriptError) onScriptError(errorMsg);
      throw error;
    }
  }, [onScriptError]);
  ...
  if (error) {
    return (
      <GlassmorphicCard variant="danger" className={`relative ${className}`}>
        <div className="flex flex-col items-center justify-center p-8 text-center min-h-[320px]">
          ...
          <h3 className="text-lg font-semibold text-white mb-2">Engine Error</h3>
  ...
  return (
    <GlassmorphicCard variant="strong" className={`relative overflow-hidden ${className}`}>
      {/* Canvas */}
      <canvas
        ref={canvasRef}
        width={width}
