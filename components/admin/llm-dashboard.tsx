  ...
  const loadMetrics = async () => {
    ...
    try {
      ...
      const aggregatedMetrics: LLMMetrics = {
        ...
        total_tokens: requests?.reduce((sum, r) => sum + (r.tokens_used || 0), 0) || 0,
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          <CardBody>
            ...
            <p className="text-2xl font-bold">{formatNumber(metrics?.total_requests || 0)}</p>
            <Progress 
        ...
        <Card>
          <CardBody>
            ...
            <p className="text-2xl font-bold">{formatNumber(metrics?.total_tokens || 0)}</p>
        ...
        <Card>
          <CardBody>
            ...
            <p className="text-tiny text-default-400">
              Avg: {formatCurrency((metrics?.total_cost || 0) / Math.max(1, metrics?.total_requests || 1))} per request
            </p>
          </CardBody>
        </Card>
        ...
        <Card>
          <CardBody>
            ...
            <Badge 
              color={metrics?.success_rate >= 95 ? 'success' : metrics?.success_rate >= 90 ? 'warning' : 'danger'}
              className="mt-2"
            >
      ...
      <Card>
        ...
        <CardBody>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <p className="text-small text-default-500 mb-2">Average Response Time</p>
              <div className="flex items-center gap-2">
                <p className="text-xl font-semibold">{metrics?.avg_response_time}ms</p>
                <Badge 
                  color={
                    metrics?.avg_response_time < 1000 ? 'success' : 
                    metrics?.avg_response_time < 3000 ? 'warning' : 'danger'
                  }
                  variant="flat"
                >
                  {metrics?.avg_response_time < 1000 ? 'Fast' : 
                   metrics?.avg_response_time < 3000 ? 'Normal' : 'Slow'}
                </Badge>
              </div>
            </div>

            <div>
              <p className="text-small text-default-500 mb-2">Requests by Provider</p>
              <div className="space-y-2">
                {Object.entries(metrics?.requests_by_provider || {}).map(([provider, count]) => (
        ...
        <CardBody>
          <div className="flex items-end gap-1 h-32">
            {metrics?.hourly_requests.map(({ hour, count }) => {
              const maxCount = Math.max(...(metrics?.hourly_requests.map(h => h.count) || [1]));
            ...
            {metrics?.top_users.map((user, index) => (
              <div key={user.user_id} className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <Badge variant="flat" color={index === 0 ? 'warning' : 'default'}>
                    #{index + 1}
                  </Badge>
                ...
                <div className="flex gap-4 text-small">
                  ...
                  <span className="text-default-500">{formatNumber(user.tokens)} tokens</span>
                </div>
              </div>
            ))}
          </div>
