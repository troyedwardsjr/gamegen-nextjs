...
const DEFAULT_CONFIG: AssetUploadConfig = {
  ...
  allowedFormats: ['png', 'jpg', 'webp', 'gif', 'wav', 'mp3', 'ogg', 'json', 'glsl'],
  allowedTypes: ['sprite', 'tileset', 'sound', 'music', 'animation', 'texture', 'shader', 'data'],
  autoOptimize: true,
...
const getFileType = (file: File): AssetType | null => {
  ...
  if (mimeType.startsWith('audio/') || ['wav', 'mp3', 'ogg', 'flac'].includes(ext || '')) {
...
const validateFile = (file: File, config: AssetUploadConfig) => {
  ...
  if (file.size > config.maxFileSize) {
    errors.push(`File size (${formatFileSize(file.size)}) exceeds limit (${formatFileSize(config.maxFileSize)})`);
  }
...
export function AssetUpload({
  ...
  const simulateUpload = useCallback(async (file: File): Promise<AssetUploadProgress> => {
  ...
  const handleRetry = useCallback((fileId: string) => {
    ...
    if (upload) {
      ...
      setUploads(prev => prev.map(u => 
        u.fileId === fileId 
          ? { ...u, progress: 0, stage: 'queued' as AssetUploadStage, error: undefined }
          : u
      ));
    }
  }, [uploads]);
  ...
  const completedUploads = uploads.filter(upload => upload.stage === 'completed');
    ...
    <div className={clsx("space-y-4", className)}>
      ...
      <GlassmorphicCard
        className={clsx(
          ...
          isDragOver && !disabled && "ring-2 ring-purple-400/60 bg-purple-500/10",
      ...
      <AnimatePresence>
        {validationErrors.length > 0 && (
          ...
          >
            <GlassmorphicCard variant="default" className="p-4 border border-red-500/30">
              <div className="flex items-center justify-between mb-2">
                <h4 className="text-sm font-semibold text-red-300">Validation Errors</h4>
                <GlassmorphicButton size="sm" variant="glass-ghost" onClick={handleClearErrors}>
                  ×
                </GlassmorphicButton>
              </div>
        ...
        {uploads.length > 0 && (
          ...
          >
            ...
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold text-white/90">Upload Progress</h3>
              <div className="flex items-center space-x-2">
                ...
                <GlassmorphicButton 
                  ...
                  disabled={completedUploads.length === 0 && failedUploads.length === 0}
                >
                  Clear
                </GlassmorphicButton>
              </div>
