...
export function AuthNavbar({ isMenuOpen, onMenuOpenChange }: AuthNavbarProps) {
  ...
  return (
    ...
    >
      <Navbar
        className={`backdrop-blur-3xl border-b transition-all duration-500 ${
          isScrolled
      ...
      >
          ...
          <NavbarBrand className="mr-4">
            ...
            >
              <motion.div
                ...
                style={{
                  background:
                    "linear-gradient(135deg, rgba(132,61,255,0.8) 0%, rgba(168,85,247,0.9) 50%, rgba(139,92,246,0.8) 100%)",
                  backdropFilter: "blur(12px)",
                  border: "1px solid rgba(132,61,255,0.4)",
                  boxShadow:
                    "0 8px 32px rgba(132,61,255,0.3), inset 0 1px 2px rgba(255,255,255,0.2)",
                }}
                ...
                whileHover={{
                  ...
                  scale: 1.15,
                  boxShadow:
                    "0 12px 48px rgba(132,61,255,0.5), inset 0 1px 2px rgba(255,255,255,0.3)",
                }}
                whileTap={{ scale: 0.95 }}
              >
                {/* Glass shine effect */}
        ...
        <NavbarContent className="hidden lg:flex gap-1" justify="center">
          <AnimatePresence>
            {(user ? authenticatedMenuItems : publicMenuItems).map(
              (item, index) => {
            ...
            <NavbarItem>
              ...
              >
                <Dropdown
                  classNames={{
                    content:
                      "glass backdrop-blur-xl border border-purple-500/20 shadow-2xl shadow-purple-500/10",
                  }}
                  ...
                  <NavbarMenuItem key={item.name}>
                    ...
                    >
                      <Link className="w-full block" href={item.href}>
                        <div
                          className={`glass p-4 rounded-xl flex items-center space-x-3 transition-all duration-300 ${
                            active
                        ...
                        >
                          <Icon
                            className={`w-5 h-5 ${active ? "text-purple-400" : "text-gray-300"}`}
                          />
                          <div className="flex flex-col">
                            <span
                              className={`font-semibold ${active ? "text-white" : "text-gray-200"}`}
                            >
                              {item.name}
                            </span>
                            {item.description && (
              ...
              <div className="mt-6 pt-4 border-t border-purple-500/20 space-y-3">
                <NavbarMenuItem>
                  <motion.div
                    ...
                    transition={{ delay: 0.4 }}
                  >
                    <Link className="w-full block" href="/auth">
                      <GlassmorphicButton
                        blur="md"
