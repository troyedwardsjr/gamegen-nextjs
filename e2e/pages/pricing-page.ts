...
export class GameGenPricingPage {
  ...
  async waitForPageLoad(): Promise<void> {
    ...
    await this.page.waitForSelector('[data-testid="pricing-plans"], .pricing-section, .plan-card', { timeout: 10000 });
  }
  ...
  async selectBillingCycle(cycle: "monthly" | "yearly"): Promise<void> {
    ...
    try {
      ...
      const toggle = this.page.locator('.billing-cycle-toggle, .toggle, .switch').first();
  ...
  async checkPricingAccuracy(): Promise<void> {
    ...
    const prices = await this.page.locator('.price, [data-testid="price"]').allTextContents();
  ...
  async verifyBillingCycleToggle(): Promise<void> {
    const billingToggle = this.page.locator('.billing-cycle-toggle, [data-testid="billing-toggle"]');
    
    if (await billingToggle.isVisible({ timeout: 2000 })) {
      await expect(billingToggle).toBeVisible();
      
      // Test toggle functionality
      const initialState = await this.page.locator('.monthly, .yearly').first().isVisible();
      await billingToggle.click();
      await this.page.waitForTimeout(500); // Allow for animation
      const afterToggle = await this.page.locator('.monthly, .yearly').first().isVisible();
  ...
  async verifyPayAsYouGoCredits(): Promise<void> {
    ...
    if (await creditsSection.isVisible({ timeout: 2000 })) {
      ...
      await expect(creditsSection).toContainText(/Credit|Pay.*Use|Per.*Action/i);
    }
  }
  ...
  async compareWithCompetitors(): Promise<void> {
    ...
    if (await comparisonSection.isVisible({ timeout: 2000 })) {
      ...
      for (const competitor of competitors) {
        const competitorElement = this.page.locator(`text=${competitor.source}`);
  ...
  async verifyUpgradePrompts(): Promise<void> {
    ...
    if (await upgradePrompts.isVisible({ timeout: 2000 })) {
      ...
      await expect(upgradePrompts).toContainText(/Recommended|Popular|Best.*Value/i);
    }
  }
  ...
  async getFAQSection(): Promise<void> {
    ...
    if (await faqSection.isVisible({ timeout: 2000 })) {
      ...
      for (const questionRegex of expectedQuestions) {
        const questionElement = this.page.locator(`text=${questionRegex.source}`);
