...
test.describe("GameGen Mobile Experience", () => {
  ...
  test.describe("Mobile Landing Page Experience", () => {
    ...
    test("should support touch gestures for feature exploration", async ({
      page,
    }) => {
      await page.setViewportSize({ width: 375, height: 667 });
      await landingPage.goto();
  ...
  test.describe("Mobile Authentication Flow", () => {
    test("should complete mobile registration flow", async ({ page }) => {
      ...
      const registrationForm = page.locator(
        '[data-testid="register-form"], form',
      );
      ...
      await page.fill(
        'input[type="email"]',
        `mobile-${timestamp}@test.gamegen.com`,
      );
    ...
    test("should handle mobile login with biometric integration", async ({
      page,
    }) => {
      await page.setViewportSize({ width: 375, height: 667 });
  ...
  test.describe("Mobile Game Creator Interface", () => {
    test("should adapt creator layout for mobile screens", async ({ page }) => {
      ...
      const chatPanel = page.locator(
        '[data-testid="vibe-chat-panel"], .chat-interface',
      );
      const editorPanel = page.locator(
        '[data-testid="editor-panel"], .editor-tabs',
      );
      const assetPanel = page.locator(
        '[data-testid="asset-panel"], .asset-library',
      );
      ...
      if (await mobileTabs.isVisible({ timeout: 2000 })) {
        ...
        if (tabCount > 1) {
          ...
          const activePanel = page.locator(
            ".active-panel, .tab-content:visible",
          );
    ...
    test("should support mobile asset library browsing", async ({ page }) => {
      ...
      const assetLibrary = page.locator(
        '[data-testid="asset-panel"], .asset-library',
      );
      ...
      if (await assetLibrary.isVisible({ timeout: 3000 })) {
        ...
        if (itemCount > 1) {
          ...
          if (firstBox && secondBox) {
            ...
            expect(firstBox.width + secondBox.width).toBeLessThanOrEqual(
              375 + 50,
            ); // Allow for margins
          }
        }
  ...
  test.describe("Mobile Performance & Optimization", () => {
    ...
    test("should handle memory constraints on mobile", async ({ page }) => {
      ...
      await creatorPage.sendChatMessage(
        "Create a complex game with many assets",
      );
  ...
  test.describe("Mobile Platform-Specific Features", () => {
    test("should handle iOS Safari specific behaviors", async ({ page }) => {
      ...
      if (
        bodyStyles.paddingTop !== "0px" ||
        bodyStyles.paddingBottom !== "0px"
      ) {
    ...
    test("should handle Android Chrome specific behaviors", async ({
      page,
    }) => {
