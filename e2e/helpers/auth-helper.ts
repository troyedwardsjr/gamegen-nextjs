...
export class GameGenAuthHelper {
  ...
  async login(user: TestUser): Promise<void> {
    ...
    await this.page.waitForSelector(
      '[data-testid="login-form"], [data-testid="register-form"], form',
      { timeout: 10000 },
    );
    ...
    await this.page.waitForURL(/\/(dashboard|creator|games)/, {
      timeout: 15000,
    });
  ...
  async registerTestUser(user: TestUser): Promise<void> {
    ...
    if (await loginForm.isVisible()) {
      ...
      const signUpToggle = this.page.locator(
    ...
    if (this.page.url().includes("/verify")) {
      console.log(
        "Email verification required - handling for GameGen test user",
      );
