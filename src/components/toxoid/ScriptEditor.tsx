...
const TOXOID_KEYWORDS = [
  'Toxoid', 'API', 'System', 'Query', 'Observer', 'Entity', 'Phases', 'Events',
  'createEntity', 'registerComponent', 'registerSingleton', 'getComponent',
  'setComponent', 'loadSprite', 'loadSpineAnimation', 'console'
];
...
export const ScriptEditor: React.FC<ScriptEditorProps> = ({
  ...
  const analyzeCode = useCallback((codeText: string) => {
    ...
    lines.forEach((line, index) => {
      ...
      if (line.includes('Toxoid.') && !TOXOID_KEYWORDS.some(keyword => line.includes(keyword))) {
        newErrors.push({
          line: lineNumber,
    ...
    <div className={`h-full flex flex-col ${className}`}>
      ...
      <div className="flex items-center justify-between p-4 border-b border-white/10">
        <div className="flex items-center space-x-2">
          ...
          {hasUnsavedChanges && (
            <GlassmorphicBadge variant="warning" size="sm">
              Unsaved
            </GlassmorphicBadge>
          )}
        </div>
      ...
      <AnimatePresence>
        {showTemplates && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
          >
            <GlassmorphicCard variant="subtle" className="p-2 w-64">
            ...
            <GlassmorphicCard variant="subtle" className="h-full">
              <div className="p-3">
                <div className="flex items-center justify-between mb-3">
                  <h4 className="text-sm font-semibold text-white/80">Issues</h4>
                  <GlassmorphicBadge 
                    variant={errors.some(e => e.type === 'error') ? "danger" : "warning"}
                    size="sm"
                  >
                    {errors.length}
                  </GlassmorphicBadge>
                ...
                <div className="space-y-2 max-h-80 overflow-y-auto">
                  {errors.map((error, i) => (
                    <GlassmorphicAlert
                      key={i}
                      variant={error.type === 'error' ? 'danger' : error.type === 'warning' ? 'warning' : 'info'}
      ...
      <div className="flex items-center justify-between px-4 py-2 border-t border-white/10 text-xs text-white/60">
        ...
        <div className="flex items-center space-x-2">
          <GlassmorphicBadge 
            variant={errors.some(e => e.type === 'error') ? "danger" : errors.length > 0 ? "warning" : "success"}
            size="sm"
          >
