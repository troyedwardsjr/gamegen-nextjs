...
import { Database, Subscription as DBSubscription } from './database';
...
export interface BillingError extends Error {
  ...
  retryable: boolean;
  user_message: string;
  action_required?: 'update_payment_method' | 'contact_support' | 'upgrade_plan';
}
 ...
 */
export const isActiveSubscription = (subscription: Subscription | null): boolean => {
...
export const getNextBillingDate = (
  ...
  if (billingCycle === 'monthly') {
    return new Date(currentEnd.getFullYear(), currentEnd.getMonth() + 1, currentEnd.getDate());
  } else {
    return new Date(currentEnd.getFullYear() + 1, currentEnd.getMonth(), currentEnd.getDate());
  }
};
