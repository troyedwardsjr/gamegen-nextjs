...
import { UserProfile as DBUserProfile, Game, Asset } from "./database";
...
export const isCompleteUserProfile = (
  profile: any,
): profile is UserProfileComplete => {
  return (
    profile &&
    "profile" in profile &&
...
export const calculateCreatorLevel = (
  stats: UserStats,
): "beginner" | "intermediate" | "advanced" | "expert" => {
  const totalCreations =
    stats.total_games_created + stats.total_assets_uploaded;
  const totalEngagement =
    stats.total_plays_received + stats.total_likes_received;
...
export const calculateReputationScore = (stats: UserStats): number => {
  ...
  const creationScore =
    stats.total_games_published * 10 + stats.total_assets_uploaded * 2;
  const engagementScore =
    stats.total_likes_received * 3 + stats.total_comments_made * 1;
  const communityScore =
    stats.total_followers * 5 + stats.total_games_forked * 2;
