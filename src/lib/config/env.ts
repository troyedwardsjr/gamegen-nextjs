...
class Environment {
  ...
  private validateAndLoadConfig(): EnvironmentConfig {
    ...
    const optionalVars = {
      ...
      NEXT_PUBLIC_CLAUDE_MODEL: process.env.NEXT_PUBLIC_CLAUDE_MODEL || 'claude-3-sonnet-20240229',
      STRIPE_SECRET_KEY: process.env.STRIPE_SECRET_KEY,
      STRIPE_WEBHOOK_SECRET: process.env.STRIPE_WEBHOOK_SECRET,
      NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY: process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY,
      NEXT_PUBLIC_TOXOID_ENGINE_URL: process.env.NEXT_PUBLIC_TOXOID_ENGINE_URL || '/toxoid',
      NEXT_PUBLIC_ASSET_CDN_URL: process.env.NEXT_PUBLIC_ASSET_CDN_URL || '/assets',
  ...
  public hasSupabaseConfig(): boolean {
    return !!(this.config.NEXT_PUBLIC_SUPABASE_URL && this.config.NEXT_PUBLIC_SUPABASE_ANON_KEY);
  }
...
export const getToxoidEngineUrl = () => env.get('NEXT_PUBLIC_TOXOID_ENGINE_URL');
...
export const validateEnvironment = () => {
  ...
  if (!env.hasSupabaseConfig()) {
    warnings.push('Supabase configuration is missing - authentication features will be disabled');
  }
  ...
  if (!env.hasStripeConfig() && env.isProduction()) {
    warnings.push('Stripe configuration is missing - payment features will be disabled');
  }
