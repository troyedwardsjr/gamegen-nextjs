...
import { ScriptValidationRequest } from "@/types/toxoid";
...
export async function POST(
  ...
  try {
    ...
    console.log(
      `[ValidationAPI] Validating script (${body.script.length} chars)`,
    );

    // Perform primary validation
    const validationResult =
      await scriptValidator.validateScript(validationRequest);
    ...
    if (body.securityCheck !== false) {
      // Default to true
      try {
        securityResult = await scriptSecurity.scanScript(
          body.script,
    ...
    if (body.compatibilityCheck === true) {
      // Default to false
      try {
        compatibilityResult = scriptSecurity.validateSandboxCompatibility(
          body.script,
        );
        checksPerformed.push("compatibility");
      } catch (compatibilityError) {
        console.warn(
          "[ValidationAPI] Compatibility check failed:",
          compatibilityError,
        );
    ...
    console.log(
      `[ValidationAPI] Validation completed in ${validationTime}ms - Valid: ${response.result!.isValid}`,
    );
    ...
    const errorMessage =
      error instanceof Error ? error.message : "Script validation failed";
    ...
    const capabilities = {
      validationChecks: [
        ...
        },
        {
          ...
          name: "Best Practices",
          description:
            "Checks adherence to ECS and game development best practices",
          enabled: true,
          configurable: false,
        },
        {
...
export async function PUT(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    console.log(
      `[ValidationAPI] Batch validating ${body.scripts.length} scripts`,
    );
    ...
    for (let i = 0; i < body.scripts.length; i++) {
      ...
      try {
        ...
        console.error(
          `[ValidationAPI] Script ${i} validation failed:`,
          scriptError,
        );
        errors.push({
          index: i,
    ...
    console.log(
      `[ValidationAPI] Batch validation completed: ${successCount} success, ${errorCount} errors in ${validationTime}ms`,
    );
...
function validateRequestBody(
  ...
  if (body.constraints) {
    ...
    if (
      body.constraints.maxMemoryMB &&
      (typeof body.constraints.maxMemoryMB !== "number" ||
        body.constraints.maxMemoryMB <= 0)
    ) {
      return {
    ...
    if (
      body.constraints.allowedAPIs &&
      !Array.isArray(body.constraints.allowedAPIs)
    ) {
      return {
    ...
    if (
      body.constraints.forbiddenPatterns &&
      !Array.isArray(body.constraints.forbiddenPatterns)
    ) {
      return {
