...
export async function POST(
  ...
  try {
    ...
    // Generate script
    console.log(
      `[ScriptAPI] Generating script for game type: ${generationRequest.gameType}, complexity: ${generationRequest.complexity}`,
    );

    const generatedScript =
      await scriptGenerator.generateScript(generationRequest);
    ...
    if (body.validateGenerated !== false) {
      // Default to true
      try {
        ...
        console.warn(
          "[ScriptAPI] Validation failed, but continuing:",
          validationError,
        );
    ...
    console.log(
      `[ScriptAPI] Successfully generated script (${generatedScript.length} chars) in ${generationTime}ms`,
    );
