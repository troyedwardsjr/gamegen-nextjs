...
import { ScriptOptimizationRequest, PerformanceProfile } from "@/types/toxoid";
...
export async function POST(
  ...
  try {
    ...
    console.log(
      `[OptimizationAPI] Optimizing script (${body.script.length} chars) with level: ${optimizationRequest.optimizationLevel}`,
    );

    // Perform optimization
    const optimizationResult =
      await scriptOptimizer.optimizeScript(optimizationRequest);
    ...
    console.log(
      `[OptimizationAPI] Optimization completed in ${optimizationTime}ms - Size: ${originalSize} -> ${optimizedSize} (${optimizationResult.compressionRatio.toFixed(2)}x)`,
    );
    ...
    const errorMessage =
      error instanceof Error ? error.message : "Script optimization failed";
...
export async function PUT(
  ...
  try {
    ...
    if (body.script.length > 1024 * 1024) {
      // 1MB limit
      return NextResponse.json(
        {
          success: false,
    ...
    console.log(
      `[OptimizationAPI] Profiling script (${body.script.length} chars)`,
    );
    ...
    return NextResponse.json(
      {
        success: false,
        error:
          error instanceof Error ? error.message : "Script profiling failed",
      ...
      optimizationLevels: [
        {
          ...
          name: "Basic",
          description:
            "Remove comments, optimize whitespace, basic variable optimization",
          features: [
            "Comment removal",
        ...
        },
        {
          ...
          name: "Aggressive",
          description:
            "Advanced optimizations including function inlining and loop optimization",
          features: [
      ...
      targetRuntimes: [
        {
          ...
          name: "QuickJS",
          description:
            "Optimize for QuickJS runtime performance characteristics",
          features: [
...
export async function PATCH(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    console.log(
      `[OptimizationAPI] Getting recommendations for script (${body.script.length} chars)`,
    );

    // Get optimization recommendations
    const recommendations =
      await scriptOptimizer.getOptimizationRecommendations(body.script);

    const analysisTime = Date.now() - startTime;

    console.log(
      `[OptimizationAPI] Generated ${recommendations.length} recommendations in ${analysisTime}ms`,
    );
...
function validateOptimizationRequest(
  ...
  if (
    ...
    !validOptimizationLevels.includes(body.optimizationLevel)
  ) {
    return {
  ...
  if (
    body.preserveComments !== undefined &&
    typeof body.preserveComments !== "boolean"
  ) {
    return {
