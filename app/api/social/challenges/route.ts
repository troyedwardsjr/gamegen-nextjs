...
import { Challenge, ChallengeParticipant, SocialDatabase } from '@/types/social';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const processedChallenges = challenges?.map(challenge => {
      const participant = user ? challenge.challenge_participants?.find(
        p => p.user_id === user.id
      ) : null;
    ...
    if (status) {
      ...
      countQuery = countQuery.in('status', ['upcoming', 'active', 'voting', 'completed']);
    }
  ...
  try {
    ...
    if (action === 'submit') {
      ...
      if (participationError || !participation) {
        return NextResponse.json(
          { success: false, error: 'You are not participating in this challenge' },
          { status: 400 }
        );
      }
      ...
      const now = new Date();
      const deadline = new Date(challenge.submission_deadline || challenge.ends_at);
    ...
    if (action === 'create') {
      ...
      if (!profile?.is_verified && profile?.subscription_tier !== 'enterprise') {
        return NextResponse.json(
          { success: false, error: 'Only verified users can create challenges' },
          { status: 403 }
        );
      }
