...
export function GameCreatorLayout() {
  ...
  const handleAssetsResize = useCallback((width: number) => {
    updatePanelState({ assetsWidth: width });
  }, [updatePanelState]);

  const handleChatCollapse = useCallback((collapsed: boolean) => {
    updatePanelState({ chatCollapsed: collapsed });
  }, [updatePanelState]);

  const handleAssetsCollapse = useCallback((collapsed: boolean) => {
      ...
      if (e.ctrlKey || e.metaKey) {
        switch (e.key) {
          ...
          case "\\": // Toggle all panels
            e.preventDefault();
            if (!isMobile) {
              const allCollapsed = panelState.chatCollapsed && panelState.assetsCollapsed;
              updatePanelState({
  ...
  }, [isMobile, panelState, handleChatCollapse, handleAssetsCollapse, updatePanelState]);
