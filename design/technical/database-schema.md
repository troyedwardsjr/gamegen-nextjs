# GameGen Platform: Database Schema Design

**Version**: 1.0  
**Date**: 2025-09-05  
**Database**: Supabase PostgreSQL with pgvector extension  
**ORM**: Direct SQL with Supabase client for type safety  

## Schema Overview

The GameGen database schema is designed to support a comprehensive game creation platform with user-generated content, AI-powered generation, real-time collaboration, and community features. The schema utilizes PostgreSQL's advanced features including JSONB for flexible data storage, pgvector for semantic search, and Row Level Security (RLS) for fine-grained access control.
...
-- ... (enable for all user-facing tables)
...
CREATE POLICY "Anyone can comment on public games" ON game_comments
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM games 
            WHERE id = game_id 
            AND (visibility = 'public' OR visibility = 'educational')
        )
    );
...
This comprehensive database schema provides the foundation for all GameGen platform features while ensuring scalability, security, and performance. The schema supports the complete feature set from basic game creation to advanced community and marketplace functionality.