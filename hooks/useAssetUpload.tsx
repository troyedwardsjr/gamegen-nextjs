...
const simulateUploadStages = async (
  ...
  const stages: AssetUploadStage[] = [
    ...
    ...(config.generateThumbnail ? ['generating_thumbnail' as AssetUploadStage] : []),
    ...(config.extractMetadata ? ['extracting_metadata' as AssetUploadStage] : []),
  ...
  const updateProgress = (stage: AssetUploadStage, progress: number, error?: string) => {
    const estimatedTimeRemaining = Math.max(0, (totalStages - currentStageIndex - 1) * 0.8);
  ...
  try {
    for (let i = 0; i < stages.length; i++) {
      ...
      if (Math.random() < 0.05 && stage !== 'completed') { // 5% chance of error
        throw new AssetError(`Failed during ${stage} stage`, 'UPLOAD_STAGE_FAILED');
      }
    }
    ...
    const asset: Asset = {
      ...
      size: file.size,
      dimensions: type === 'sprite' || type === 'tileset' ? { width: 256, height: 256 } : undefined,
      quality: 'high',
      tags: [],
      collections: [],
      categories: [type],
...
export function useAssetUpload(): UseAssetUploadReturn {
  ...
  const updateProgress = useCallback((newProgress: AssetUploadProgress) => {
    setProgress(prev => {
      ...
      if (existing) {
        return prev.map(p => p.fileId === newProgress.fileId ? newProgress : p);
      } else {
    ...
    try {
      ...
      for (const file of files) {
        ...
        try {
          ...
          if (error instanceof AssetError && error.code === 'UPLOAD_CANCELLED') {
