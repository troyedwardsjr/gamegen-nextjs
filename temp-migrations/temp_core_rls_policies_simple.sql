-- Migration: Create RLS Policies for Core Tables (Simplified)
-- Description: Address security advisor findings with basic but secure RLS policies
-- Date: 2025-09-09
...
COMMENT ON POLICY "Users can view public games and manage their own" ON games IS 'Public games are viewable by all, private games only by creator';
COMMENT ON POLICY "Public access to user follows" ON user_follows IS 'Social follows are public for discovery';